    ...
    saveStepButton.addEventListener('click', async () => {
        ...
        try {
            ...
            headers: {
                ...
                'X-User-Full-Name': '{{ current_user.full_name }}',
...
function handleDeleteScriptClick(event) {
    ...
    fetch('/call-center/admin/delete_script', {
        method: 'POST',
        headers: {
            'X-User-Name': '{{ current_user.username }}',
            'X-User-Role': '{{ current_user.role }}',
    ...
    .then(response => {
        ...
        return response.json().then(data => {
            // Return both the status and data
            return { ok: isSuccess, data: data };
        });
    })
    .then(result => {
        // Change this line - use result.ok instead of response.ok
        if (result.ok) {
    ...
    .catch(error => {
        ...
        showMessage('Ошибка сети или сервера при удалении скрипта.', 'error');
    });
}
